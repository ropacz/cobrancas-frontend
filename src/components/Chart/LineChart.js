...
const LineChart = ({
  ...
  const pointsInLine = data.map((element) => {
    const x = (element.x / maximumXFromData) * chartWidth + padding;
    const y =
      chartHeight - (element.y / maximumYFromData) * chartHeight + padding;
    return { x: x, y: y };
  ...
  return (
    ...
    >
      ...
      <polyline
        ...
        points={points}
      />
      <PointsInLine data={pointsInLine} />
    </svg>
  );
};
